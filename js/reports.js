...
function exportExcelReport() {
  ...
  try {
    ...
    XLSX.writeFile(wb, `Ripple-Report-${reportDateStamp()}.xlsx`);
...
function buildPrintReportHTML() {
  ...
  return `
    ...
    <p class="print-footer">Ripple · RateGain Mentorship &amp; Peer Learning Ecosystem</p>`;
}
  ...
  html2canvas(document.getElementById("insights-report-area"), { backgroundColor: "#ffffff", scale: 2 })
    .then((canvas) => {
      canvas.toBlob((blob) => {
        ...
        a.href = url;
        a.download = `Ripple-Insights-${reportDateStamp()}.png`;
