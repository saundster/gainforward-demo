...
function exportExcelReport() {
  ...
  try {
    ...
    XLSX.writeFile(wb, `Waypoint-Report-${reportDateStamp()}.xlsx`);
...
function buildPrintReportHTML() {
  ...
  return `
    ...
    <p class="print-footer">Waypoint · RateGain Mentorship &amp; Peer Learning Ecosystem</p>`;
}
  ...
  html2canvas(document.getElementById("insights-report-area"), { backgroundColor: "#ffffff", scale: 2 })
    .then((canvas) => {
      canvas.toBlob((blob) => {
        ...
        a.href = url;
        a.download = `Waypoint-Insights-${reportDateStamp()}.png`;
